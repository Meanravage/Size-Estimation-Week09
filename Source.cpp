    ...
    for (int r = 0; r < BOARDSIZE; r++)
        for (int c = 0; c < BOARDSIZE; c++)
            for(int z = 0; z < BOARDSIZE; z++)
            {
                fin >> board[r][c][z];
                assert(!fin.fail());
                assert(board[r][c][z] == BOARDX ||
                    board[r][c][z] == BOARDO ||
                    board[r][c][z] == BOARDDOT);
            }
...
bool write(const char board[][][BOARDSIZE], const char* fileName)
{
    ...
    // write my 9 symbols 3 times in 3 levels
    for (int r = 0; r < BOARDSIZE; r++)
        for (int c = 0; c < BOARDSIZE; c++)
            for(int z = 0; z < BOARDSIZE; z++)
                fout << board[r][c][z] << (c == 2 ? '\n' : ' ') << (z == 2 ? '\n' : ' ');
...
void display(const char board[][][BOARDSIZE])
{
    ...
    for (int r = 0; r < BOARDSIZE; r++)
    {
        ...
        for (int c = 0; c < BOARDSIZE; c++)
        {
            ...
            for (int z = 0; c < BOARDSIZE; z++)
            {
                // display a space for the dot
                if (board[r][c][z] == BOARDDOT)
                    cout << "   ";
                else
                    cout << " " << board[r][c][z] << " ";
